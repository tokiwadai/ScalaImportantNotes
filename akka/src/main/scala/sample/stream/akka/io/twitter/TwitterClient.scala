...
object TwitterClient {
  ...
  def retrieveTweets(term: String): List[Tweet] = {
    val query = new Query(term)
//    query.setCount(100)
    val result: Iterator[Status] = getInstance.search(query).getTweets.asScala.iterator
    val tweets: List[Tweet] = result.map(status => Tweet(Author(status.getUser.getScreenName), status.getText))
      .toList
    tweets
  }
}
